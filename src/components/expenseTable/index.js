...
import { deleteTransaction, fetchAllExpenses } from "../../helpers/apiHelper";
import "./index.scss";
...
import { removeExpense, setExpenses } from "../../state/actions/expense";

const Accounts = () => {
  //load redux state and dispather
  const accounts = useSelector((state) => state.expense.expenses).sort(
    function ( a,b) {
    ...
    <>
      ...
      <div className="accountTable">
        <div className="accountTableHeader">
          <div className="accountTableHeaderText">Expense Records </div>
          <div className="accountTableHeaderButton">
            <AddExpenseModalModalLauncher rerenderTable={fetchAndSetAccounts} />
          </div>
        </div>
        <Table>
          <tbody>
                    ...
                    >
                      <Dropdown.Item
                        onClick={() => {
                          dispatch({
                            type: setIntent,
                            payload: { key: "editExpense", value: account },
                          });
                          toggleEditModal();
                        }}
                      >
                      ...
                      <Dropdown.Item
                        onClick={() => {
                          deleteTransaction(account._id).then(() => {
                            dispatch({ type: removeExpense, payload: account });
                          });
                        }}
                      >
                        Delete
